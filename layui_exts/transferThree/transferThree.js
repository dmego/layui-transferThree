...
layui.define('table', function (exports) {
    ...
    var $ = layui.$,
        ...
        transferThree = {
            index: layui.transferThree ? (layui.transferThree + 1000) : 0,
            idData: [], //ID池
            data: [], //原始数据
            options: [],
        ...
        Class = function (options) {
            var that = this
            that.index = transferThree.index ? transferThree.index : ++transferThree.index
            that.config = options
            transferThree.options = options
    ...
    Class.prototype.render = function () {
        ...
        transferThree.idData.push(that.index)
        transferThree.data = [];
        transferThree.data.push({
            id: LEFT_TABLE+that.index,
            data: (options.data[0] ? options.data[0] : [])
        })
        transferThree.data.push({
    ...
    */
    function datas(type) {
        ...
        var options = transferThree.options
        options.data = datasChecked(_d);
        transferThree.render(options)
    }
    ...
    table.on('checkbox('+FILTER+')', function (obj) {
        var idData = transferThree.idData,
    ...
    transferThree.render = function (options) {
        var inst = new Class(options)
        return thisRate.call(inst)
    }
    exports(MOD_NAME, transferThree)
})